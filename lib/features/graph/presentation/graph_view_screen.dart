...
class _GraphViewScreenState extends State<GraphViewScreen> {
  ...
  void _initializeServices() {
    ...
    // Initialize painter with empty data
  ...
  void _handleNodeTap(GraphNode node) {
    if (node.isGhost) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('"${node.label}" henüz oluşturulmamış bir hayalet nottur.'),
        ),
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: Icon(_isPhysicsActive ? Icons.pause : Icons.play_arrow),
      ...
      floatingActionButton: Column(
        ...
        children: [
          FloatingActionButton(
            ...
            child: Icon(
              _isPhysicsActive ? Icons.pause : Icons.play_arrow,
              color: Colors.white,
            ),
            onPressed: () => _togglePhysics(!_isPhysicsActive),
